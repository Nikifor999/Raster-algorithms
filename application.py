...
class CoordinateGridApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Координатная сетка на Canvas")
    ...
    def create_input_fields(self):
        ...
        self.draw_button = tk.Button(self.input_frame, text="Draw", command=self.draw_line)
        self.draw_button.grid(row=4, columnspan=2)
        self.clear_button = tk.Button(self.input_frame, text="Clear", command=self.clear_canvas)
    ...
    def draw_line(self):
        ...
        try:
            ...
            messagebox.showinfo("Время выполнения", f"Время выполнения алгоритма: {execution_time:.2f} мс")
